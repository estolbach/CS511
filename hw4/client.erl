    ...
    case maps:is_key(ChatName, State#cl_st.con_ch) of
        ...
        true ->
            ...
            receive
                ...
                % remove the chatroom from its list of chatrooms.
                Result = maps:remove(ChatName, State#cl_st.con_ch),
                State1 = State#cl_st{con_ch = Result},
                {ok, State1}
            end
    end.
...
do_new_nick(State, Ref, NewNick) ->
    % check if new nick name is same as current
    case NewNick /= State#cl_st.nick of
        false -> {err_same, State}; % if nick are equal throw error
        true ->
...
do_msg_send(State, Ref, ChatName, Message) ->
    ...
    ChatPID = case maps:find(ChatName, State#cl_st.con_ch) of
        {ok, Chat} -> Chat;
        error -> pass
    end,
